...
def build_points(source_path: Path, chunks: list[str], vectors: list[list[float]], extra: dict | None = None) -> list[dict]:
    payload_base = {
    ...
    for index, (text, vector) in enumerate(zip(chunks, vectors)):
        ...
        points.append({"id": point_id(str(source_path.resolve()), index), "vector": vector, "payload": payload})
    return points



def index_documents(full_reindex: bool = False) -> None:
    ...
    for file_path in files:
        ...
        for part in parsed_parts:
            chunks = chunk_text(part.text, settings.rag_chunk_chars, settings.rag_chunk_overlap)
            merged_chunks.extend(chunk.text for chunk in chunks)
        ...
        store.replace_document(str(file_path.resolve()))
        points = build_points(file_path, merged_chunks, vectors)
        ...
        indexed_count += 1
        logger.info("Indexed %s chunks for %s", len(points), file_path)

    save_state(state)
    logger.info("Indexing complete. Files indexed: %s", indexed_count)
...
def main() -> int:
    ...
    index_documents(full_reindex=args.full_reindex)
    return 0
