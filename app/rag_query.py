...
def extract_citations(results: list[dict]) -> list[dict]:
    ...
    for item in results:
        ...
        label = item.get("citation", f"{file_name}#{chunk_index}").strip("[]").strip()
...
def build_qdrant_filter(filetype, file, since, tag):
    if not any([filetype, file, since, tag]):
        return None
...
def stream_query(
    ...
    """
    Streams output to stdout as newline-delimited JSON events:
      {"type": "token",    "content": "..."}   — one per LLM token
      {"type": "citations","data": [...]}       — once at the end
    ...
    try:
        ...
        if not context.strip():
            msg = "Ik weet het niet op basis van de huidige index. Voeg relevante documenten toe en indexeer opnieuw."
            print(json.dumps({"type": "token", "content": msg}), flush=True)
            print(json.dumps({"type": "citations", "data": []}), flush=True)
            return

        messages = build_messages(question, context)
        endpoint = f"{settings.lmstudio_url.rstrip('/')}/v1/chat/completions"

        with requests.post(
            endpoint,
            json={"model": settings.lmstudio_model, "messages": messages, "stream": True},
            stream=True,
            timeout=600,
        ) as resp:
        ...
        print(json.dumps({"type": "citations", "data": citations}), flush=True)
...
def run_query(
    ...
    """Non-streaming fallback. Returns dict with answer + citations."""
    settings = load_settings()
    top = top_k or settings.rag_top_k
    ...
    response = requests.post(
        endpoint,
        json={"model": settings.lmstudio_model, "messages": messages, "stream": False},
        timeout=600,
    )
    response.raise_for_status()
    answer = response.json()["choices"][0]["message"]["content"]

    return {"answer": answer.strip(), "citations": extract_citations(payloads)}
...
def main() -> int:
    ...
    parser.add_argument("--stream",    action="store_true", help="Stream tokens as NDJSON to stdout")
    args = parser.parse_args()

    if args.question:
        if args.stream:
            # Streaming mode: used by the Node.js frontend
            stream_query(
                args.question,
                top_k=args.top_k,
            ...
            # Non-streaming: single JSON object, used for quick CLI testing
            try:
                result = run_query(
                    args.question,
                    top_k=args.top_k,
    ...
    # Interactive mode (no question arg)
    while True:
        try:
